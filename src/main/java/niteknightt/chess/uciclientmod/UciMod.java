package niteknightt.chess.uciclientmod;
...
public class UciMod {
    ...
    public <T> UCIResponse<T> command(String cmd, Function<List<String>, T> commandProcessor, Predicate<String> breakCondition, long timeout)  {
        CompletableFuture<List<String>> command = supplyAsync(() -> {
            ...
            try {
                logger.debugInput(gameId, cmd);
                logger.debugInput(gameId, "isready");
                ...
                while ((line = reader.readLine()) != null) {
                    ...
                    output.add(line);
                    logger.debugOutput(gameId, line);
                    if (breakCondition.test(line)) {
                        break;
                    }
                }
            } catch (IOException e) {
                logger.debugOutput(gameId, "[unknown-command]");
                throw new UCIUncheckedIOException(e);
            } catch (RuntimeException e) {
                logger.debugOutput(gameId, "[unexpected-token]");
                throw new UCIRuntimeException(e);
            }
            return output;
        });
